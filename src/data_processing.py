...
def load_and_preprocess_data(
    ...
    for line in lines:
        word = " ".join(line.split(" ")[:-2]).lower()
        procesed_word = f"{start_token}{word}{end_token}"
        for i in range(len(procesed_word)-1):
            bigrams.append((procesed_word[i], procesed_word[i+1]))
